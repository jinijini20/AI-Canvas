...
이과정에서 저는 스토리보드(장면번호/줄거리/구도/카메라각도)를 입력하면 stable diffusion 3.5영어프롬프트가 나오는 프롬프트와 

스토리보드(장면번호/줄거리/구도/카메라각도)를 입력하면 stable diffusion 3.5 한글 프롬프트를 출력하고 번역한 프롬프트 이 두가지를 시도해보았습니다.

이 두가지 실험에 대한 내용은 이문서에있습니다.

이 두가지를 시도해본결과 큰차이를 보이지않아 스토리보드(장면번호/줄거리/구도/카메라각도)를 입력하면 stable diffusion3.5 영어프롬프트가 나오는 프롬프트를 선택했습니다.

생성된 프롬프트로 이미지생성은 comfyui환경에 stable diffusion3.5모델을 사용했습니다.
